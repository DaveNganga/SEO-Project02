...
class UserReg(db.Model): #create database for registering
      ...
      def __repr__(self):
            return f"UserReg('{self.username}', '{self.email}')"

class User(db.Model): #creation of database for subscription data
      id = db.Column(db.Integer, primary_key=True)
...
def register_account():
      ...
      if form.validate_on_submit(): #what happens when the user signs up for an account
          user = UserReg(username=form.username.data, email=form.email.data, password=form.password.data)
          db.session.add(user)
          db.session.commit()
